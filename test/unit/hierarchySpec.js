	...
	describe('fngHierarchyListCtrl Controller', function() {
		...
		describe('onDrop behaviour', function() {
			...
			beforeEach(function() {
				...
				spyOn(scope, 'getIndex').andCallThrough();
			...
			it('onDrop should call utils.getIndex', function() {
				scope.onDrop(event, ui);
				expect(scope.getIndex).toHaveBeenCalledWith(scope.record, scope.model, childElementNo);
			});
		...
		describe('parsePath behaviour', function() {
			...
			beforeEach(function() {
				spyOn(scope, 'parsePath').andCallThrough();
				spyOn(scope, 'createFormSchema').andCallThrough();
			});
			...
			it('onOver should call $apply', function() {
				scope.parsePath();
				expect(scope.createFormSchema).toHaveBeenCalled();
			});
	...
	describe('fngHierarchyChild directive', function() {
		...
		beforeEach(function() {
			inject(function($rootScope, $compile, $controller) {
                ...
                hierarchy = scope.createFormSchema(recordData.Hierarchy);

                scope.node = hierarchy[0];
	...
	describe('fngHierarchyChildCtrl Controller', function() {
		...
		beforeEach(function() {
			inject(function($rootScope, $controller) {
                ...
                var parentCtrl = $controller("fngHierarchyListCtrl", {
                    $scope: scope
                });

                scope.record = recordData;

                scope.hierarchy = scope.createFormSchema(recordData.Hierarchy);
                scope.node = scope.hierarchy[0].content[0];
...
//
		...
		describe('removeLine behaviour', function() {
			...
			it('scope.removeLine should be called', function() {
                ...
                scope.node = {elementNo: 87345};
				scope.removeLine(scope.model, scope.node.elementNo);
				expect(scope.removeLine).toHaveBeenCalledWith(scope.model, scope.node.elementNo);

			});

			it('removeLine should error and $emit error if content scope.node.content is not empty', function() {
				scope.node = {elementNo: 87345, content: [{stuff:1}]};
				scope.removeLine(scope.model, scope.node.elementNo);
				expect(scope.$emit).toHaveBeenCalled();
			});
